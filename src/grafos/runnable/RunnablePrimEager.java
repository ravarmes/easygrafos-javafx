 ...
 * @author 
 */
public class RunnablePrimEager implements Runnable {
    ...
    private boolean executar;     // variavel booleam que verifica se a Thread que implementa o RunnablePrimEager esta ativa ou não 
     ...
     */
    public RunnablePrimEager(GrafoPonderado grafo, Thread main, Group componentesLine, Group componentesCircle, 
        ...
        RunnablePrimEager.grafo = grafo;
        this.main = main;
        RunnablePrimEager.componentesLine = componentesLine;
        RunnablePrimEager.componentesCircle = componentesCircle;
        RunnablePrimEager.codigoPane = codigoPane;
        RunnablePrimEager.tableViewMST = tableViewMST;
        RunnablePrimEager.tableViewPQ = tableViewPQ;
        RunnablePrimLazy.tableViewMarcado = tableViewMarcado;
        RunnablePrimEager.variaveis = variaveis;
        this.executar = true;
    ...
    public void run() {
        try {
            //Instancia um objeto do tipo AlgoritmoPrimEager. No seu construtor o algoritmo PrimEagerItemTablewViewMST começa a ser realmente executado
            AlgoritmoMSTPrimEager AlgoritmoPrimEager = new AlgoritmoMSTPrimEager(this.grafo, this, this.main, this.v0);
        } catch (InterruptedException ex) {
            Logger.getLogger(RunnablePrimEager.class.getName()).log(Level.SEVERE, null, ex);
        }
        try {
    ...
    /**
     *
     * @param listPrimEagerItemTablewViewMST Lista com objetos do tipo
     * PrimEagerItemTablewViewMST, para preencher a tabela de parâmetros
     * (tableViewPropriedades)
     * @param validade true se a Thread que implementa a runnablePrimEager estiver
     * ativa e false para não ativa.
     */
    public static void alterarTablewViewMST(List<PrimEagerItemTablewViewMST> listPrimEagerItemTablewViewMST, boolean validade) {
        if (validade) {
            ObservableList<PrimEagerItemTablewViewMST> ObservableListTabela = FXCollections.observableArrayList(listPrimEagerItemTablewViewMST);
            Platform.runLater(() -> {
                RunnablePrimEager.tableViewMST.setItems(ObservableListTabela);
                RunnablePrimEager.tableViewMST.refresh();
            });
        }
    }
    ...
    /**
     *
     * @param listPrimEagerItemTablewViewPQ Lista com objetos do tipo
     * PrimEagerItemTablewViewPQ, para preencher a tabela de parâmetros
     * (tableViewPropriedades)
     * @param validade true se a Thread que implementa a runnablePrimEager estiver
     * ativa e false para não ativa.
     */
    public static void alterarTablewViewPQ(FilaPrioridadeMinIndex<PrimEagerItemTablewViewPQ> filaPrimEagerItemTablewViewPQ, boolean validade) {
        if (validade) {
            
            List<PrimEagerItemTablewViewPQ> listPrimEagerItemTablewViewPQ = new ArrayList<PrimEagerItemTablewViewPQ>();
//            FilaPrioridadeMinIndex<PrimEagerItemTablewViewPQ> filaPQaux = new FilaPrioridadeMinIndex<>(filaPrimEagerItemTablewViewPQ.getMaxN());
//            
//            filaPQaux = filaPrimEagerItemTablewViewPQ;
//            
//            while(!filaPQaux.isEmpty()){
//                listPrimEagerItemTablewViewPQ.add(filaPQaux.minChave());
//                filaPQaux.removeMin(); 
//            }

            for(int i = 0; i < filaPrimEagerItemTablewViewPQ.getMaxN(); i++){
                //tentar ver se tem jeito melhor pra fazer, a não ser que as chaves sempre sejam os números dos vértices
                if(filaPrimEagerItemTablewViewPQ.contem(i)){
                    listPrimEagerItemTablewViewPQ.add(filaPrimEagerItemTablewViewPQ.chaveDe(i));
                }
            }
            
            Collections.sort(listPrimEagerItemTablewViewPQ, new Comparator<PrimEagerItemTablewViewPQ>() {
                @Override
                public int compare(PrimEagerItemTablewViewPQ p1, PrimEagerItemTablewViewPQ p2) {
                    return p1.getPeso().compareTo(p2.getPeso());
                }
            });
            
            ObservableList<PrimEagerItemTablewViewPQ> ObservableListTabela = FXCollections.observableArrayList(listPrimEagerItemTablewViewPQ);
            Platform.runLater(() -> {
                RunnablePrimEager.tableViewPQ.setItems(ObservableListTabela);
                RunnablePrimEager.tableViewPQ.refresh();
            });
        }
    }
    ...
    /**
     *
     * @param listMarcado Lista com objetos do tipo
     * PrimMarcado, para preencher a tabela de parâmetros
     * (tableViewPropriedades)
    ...
    public static void exibirVariaveis(String status, boolean validade) {
        if (validade) {
            Platform.runLater(() -> {
                RunnablePrimEager.variaveis.setText(status);
            });
        }
    }
    ...
    /**
     *
     * @param listPrimEagerParametros Lista com objetos do tipo
     * PrimEagerItemTablewViewMST, para preencher a tabela de parâmetros
     * (tableViewPropriedades)
     * @param validade true se a Thread que implementa a runnablePrimEager estiver
     * ativa e false para não ativa
     */
    public static void iniciarTabela(List<PrimEagerItemTablewViewMST> listPrimEagerParametros, boolean validade) {
        if (validade) {
            ObservableList<PrimEagerItemTablewViewMST> ObservableListTabela = FXCollections.observableArrayList(listPrimEagerParametros);
            Platform.runLater(() -> {
                RunnablePrimEager.tableViewMST.setItems(ObservableListTabela);
            });
        }
            ...
            for (i = 0; i < grafo.V(); i++) {
                ...
                for (j = 0; j < count; j++) {
                    ...
                    if (grafo.getAdj()[i].get(j).isUtilizada()) {
                        RunnablePrimEager.colorirResultado(grafo.getAdj()[i].get(j).getV1().getId(), grafo.getAdj()[i].get(j).getV2().getId(), validade);
                    }
                }
            }
        }
                ...
                public Void call() throws InterruptedException {
                    ...
                    while ((i <= 1) || (j <= 1)) {
                       //System.out.println("Teste" + i);
                        ...
                        Thread.sleep(AlgoritmoMSTPrimEager.velocidade/40);
                    }
                    return null;
                }
            };
    ...
    public static void showSuccess(boolean validade) {
        if (validade) {
            Platform.runLater(() -> {
            RunnablePrimEager.alertMsg = new Alert(Alert.AlertType.INFORMATION);
            RunnablePrimEager.alertMsg.setHeaderText("Sucesso!");
            RunnablePrimEager.alertMsg.setContentText("Suceeso ao encontrar a árvore geradora mínima do grafo!");
